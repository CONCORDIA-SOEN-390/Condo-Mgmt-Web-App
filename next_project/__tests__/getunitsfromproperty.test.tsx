...
import { POST } from "../src/app/api/getunitsfromproperty/route"; // Replace 'your-file-name' with the actual file name
...
describe("GET function", () => {
  ...
  it("should return units data if property_id is provided and no error occurs", async () => {
    ...
    const req = {
      json: jest.fn().mockResolvedValue({ ownerId: 1 }), // Adjust ownerId as needed
    };
    require("@supabase/supabase-js").createClient.mockReturnValueOnce({
      from: jest.fn(() => ({
        select: jest.fn(() => ({
