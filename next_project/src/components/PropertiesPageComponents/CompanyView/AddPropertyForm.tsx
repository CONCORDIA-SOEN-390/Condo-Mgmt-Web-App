...
function AddPropertyForm({userId}) {
  ...
  const [dimension, setDimension] = useState(""); // Added dimension state
  const [errorMessage, setErrorMessage] = useState("");

  const handleConfirm = async () => {
    if (propertyName.trim() === "" || address.trim() === "" || dimension.trim() === "") {
      ...
      <div className="h-screen">
        <div className="bg-sky-100 min-h-screen p-5">
          ...
          <div>
            ...
            <div className="mb-4">
              <label htmlFor="dimension" className="block text-sm font-bold text-blue-700 mb-2">Dimension</label>
              <input
                  className="appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                  type="text"
                  name="dimension"
